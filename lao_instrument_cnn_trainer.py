...
class Config:
    ...
    MODEL_SAVE_PATH = "lao_instruments_model_cnn"
...
def load_data_for_cnn():
    ...
    for parent in parent_dirs:
        if parent.startswith('khean-') or parent.startswith('khean_') or parent == 'khean':
            instrument_class = 'khean'
        elif parent.startswith('khong-vong-') or parent.startswith('khong_vong_') or parent == 'khong_wong':
...
# Build CNN model with temporal attention
...
def export_to_onnx(model, filepath_base, class_labels=None, input_names=None, output_names=None):
...
def main():
    ...
    # Export to ONNX
    export_to_onnx(model, Config.MODEL_SAVE_PATH, class_labels=classes)
    ...
    print("\nNext step: Create Streamlit app to use this model")
