...
def load_openvino_model(xml_path, device="GPU"):
    """Load OpenVINO IR model and compile for specified device"""
    try:
        ...
        # Compile the model for the specified device (GPU for Intel Arc)
...
def infer_openvino(compiled_model, input_data):
    """Perform inference using OpenVINO compiled model"""
    try:
        # Get input and output layers
        infer_request = compiled_model.create_infer_request()
        
        # Prepare input data (ensure correct shape and type)
        ...
        print(f"Error during OpenVINO inference: {str(e)}")
        return None
...
def export_model(model, filepath_base, class_labels=None):
    """Export model to TFLite, ONNX, and OpenVINO IR formats"""
    ...
    try:
        ...
        input_signature = [tf.TensorSpec([None] + list(model.input.shape[1:]), tf.float32, name="input")]
        ...
        try:
            ...
            xml_path = f"{filepath_base}.xml"
            bin_path = f"{filepath_base}.bin"
            
            # Run model conversion
            convert_model(
                model_path=onnx_path,
                output_dir=os.path.dirname(filepath_base),
                model_name=os.path.basename(filepath_base),
                input_shape=[1] + list(model.input.shape[1:]),
                compress_to_fp16=True  # Optimize for Intel Arc GPU
            )
            
            print(f"OpenVINO IR model saved at: {xml_path} and {bin_path}")
        except Exception as e:
            print(f"Error converting to OpenVINO IR: {str(e)}")
...
def predict_and_analyze(model, X_test, y_test, classes, file_paths=None, models=None, use_openvino=False, openvino_model_path=None):
    """Make predictions and analyze model performance in detail"""
    print("\nDetailed Model Analysis:")
    
    if use_openvino:
        print("Using OpenVINO inference")
        if openvino_model_path is None:
            raise ValueError("OpenVINO model path must be provided when use_openvino=True")
        
        # Load OpenVINO model
        compiled_model = load_openvino_model(openvino_model_path)
        if compiled_model is None:
            raise RuntimeError("Failed to load OpenVINO model")
        
        # Perform inference
        y_pred = np.zeros((X_test.shape[0], len(classes)), dtype=np.float32)
        for i in tqdm(range(X_test.shape[0]), desc="OpenVINO inference"):
            input_data = np.expand_dims(X_test[i], axis=0)
            pred = infer_openvino(compiled_model, input_data)
            if pred is not None:
                y_pred[i] = pred
    else:
        # Existing TensorFlow/ensemble prediction
...
def main():
    ...
    # Flag to enable OpenVINO inference
    USE_OPENVINO = True  # Set to False to use TensorFlow
    ...
    if Config.USE_ENSEMBLE:
        ...
        # Detailed analysis
        openvino_model_path = f"{Config.MODEL_SAVE_PATH}_ensemble_1.xml" if USE_OPENVINO else None
        ensemble_report, ensemble_cm, ensemble_preds = predict_and_analyze(
            models[0], X_test, y_test, classes, file_paths_test, 
            models=models if not USE_OPENVINO else None,
        ...
        ensemble_meta = {
            ...
            "accuracy": float(ensemble_accuracy) if not USE_OPENVINO else None,
        ...
        # Detailed analysis
        openvino_model_path = f"{Config.MODEL_SAVE_PATH}.xml" if USE_OPENVINO else None
        report, cm, preds = predict_and_analyze(
            model, X_test, y_test, classes, file_paths_test,
            use_openvino=USE_OPENVINO, openvino_model_path=openvino_model_path
        )
    ...
    if Config.USE_ENSEMBLE:
        if not USE_OPENVINO:
            ...
            test_model_func = None  # Handled in predict_instrument
            selected_model = models[0]
        openvino_model_path = f"{Config.MODEL_SAVE_PATH}_ensemble_1.xml"
    else:
        test_model_func = model.predict if not USE_OPENVINO else None
        selected_model = model
        openvino_model_path = f"{Config.MODEL_SAVE_PATH}.xml"
    ...
    def predict_instrument(audio_path, true_label=None, use_openvino=False, openvino_model_path=None, classes=None):
        try:
            # Load audio
            ...
            if use_openvino:
                if openvino_model_path is None or classes is None:
                    raise ValueError("OpenVINO model path and classes must be provided when use_openvino=True")
                
                # Load and infer with OpenVINO
                compiled_model = load_openvino_model(openvino_model_path)
                if compiled_model is None:
                    raise RuntimeError("Failed to load OpenVINO model")
                
                prediction = infer_openvino(compiled_model, specs)
                if prediction is None:
                    raise RuntimeError("OpenVINO inference failed")
            else:
                # Use TensorFlow/ensemble prediction
            ...
            result = {
                ...
                'all_probabilities': {
                    classes[i]: float(prob) for i, prob in enumerate(prediction[0])
                },
                'true_label': true_label
            }
